(function () {
    ...
    grid.innerHTML = filtered.map((a, i) => {
      ...
      <div class="acc-card" style="--tier-color: var(${a.tierVar}); animation-delay:${Math.min(i * 40, 400)}ms">
        ...
        <div class="acc-body">
          ...
          <div class="acc-foot">
            ${a.originalPrice ? `<span class="price-was">${fmt(a.originalPrice)}</span><span class="price-badge">10% OFF</span>` : ''}<span class="price">${fmt(a.price)}</span>
  ...
  /* ---------- Support Chat ---------- */
  const SUPPORT_FAQS = [
    { keys: ['recharge code', 'give.*code', 'free code', 'share.*code', 'what.*code', 'send.*code', 'promo code', 'give.*promo', 'free promo'], reply: '**I can\'t share codes or promos** — they\'re distributed during TikTok lives, giveaways, and Discord events. Follow https://www.tiktok.com/@ghxstlyfn so you don\'t miss the next drop!' },
    { keys: ['free account', 'give.*account', 'free.*fortnite', 'no money', 'broke', 'without.*pay', 'without.*money', 'gift.*account'], reply: '**I can\'t give out free accounts** — every listing has a price. But accounts start as low as **$2**, so they\'re super affordable. Recharge codes are dropped during TikTok lives and giveaways if you want a free shot!' },
    { keys: ['cheapest', 'cheapest.*account', 'lowest.*price', 'least.*expensive', 'budget', 'affordable', 'under.*5', 'under.*10', 'cheap.*account', 'how cheap'], reply: 'Great news — accounts start at just **$2**! We have 23 accounts in stock right now, ranging from **$2 to $60**. The cheapest ones still have tons of skins. Browse the listings and filter by price to find your perfect locker.' },
    { keys: ['most expensive', 'best.*account', 'most.*skins', 'biggest.*locker', 'top.*account', 'best.*value', 'most.*skins.*price'], reply: 'Our best account right now has **504 skins** for **$60** — that\'s the max price cap. We also have accounts with 3000+ skins at insane prices. Check the listings and sort by skins to see the heavy hitters.' },
    { keys: ['how many.*account', 'total.*account', 'what.*available', 'what.*stock', 'inventory', 'listings'], reply: 'We currently have **23 accounts** in stock, ranging from 50 to 3000+ skins. Prices go from **$2 to $60**. New accounts drop during TikTok lives — follow https://www.tiktok.com/@ghxstlyfn for restock alerts.' },
    { keys: ['price range', 'how much.*cost', 'what.*price', 'price.*account', 'cost.*account', 'how much.*account'], reply: 'Accounts range from **$2 to $60** — that\'s our hard cap. Every account is priced fairly based on its skin count and value. Use the price filters on the store to find accounts in your budget.' },
    ...
    { keys: ['code', 'recharge', 'balance', 'top up', 'topup', 'wallet', 'add money', 'fund', 'how.*balance'], reply: '**How to add balance:**\n\n1. Get a recharge code (from TikTok lives, giveaways, or the store owner)\n2. Open your **wallet** on the store\n3. Enter the code — it adds instantly to your balance\n\nEach code works **one time only**. Your balance is saved to your account and works on any device.' },
    { keys: ['warranty', 'refund', 'locked', 'recover', 'banned', 'guarantee', 'problem', 'issue', 'broken', 'wrong.*account'], reply: '**Every account comes with a 48-hour warranty.**\n\nIf you get locked out within 48 hours of purchase, open a ticket and we\'ll give you a **replacement account** or a **full refund** — your choice.\n\n[Open a support ticket](https://discord.com/channels/@me)' },
    { keys: ['promo', 'discount', 'sale', 'coupon', 'code.*off', '%', 'deal', 'offer'], reply: '**Promo codes** give you a percentage discount at checkout!\n\nTo use one: go to checkout, enter your promo code in the field, and press **Apply**. The discount shows instantly.\n\nEach promo is **single-use** — once it\'s used, it\'s gone. Follow our TikTok for new promos: https://www.tiktok.com/@ghxstlyfn' },
    { keys: ['legit', 'scam', 'trust', 'safe', 'real', 'fake', 'secure', 'reliable'], reply: '**100% legit — here\'s why:**\n\n• Wallet balance, codes, and purchases are all **server-side** — nothing can be faked from your browser\n• Order codes are issued **instantly** — no waiting around\n• Every account has a **48-hour warranty**\n• Real humans answer support tickets **24/7**\n• We have verified reviews from real buyers\n\nYour money and account are safe with us.' },
    { keys: ['owner', 'admin', 'human', 'support', 'contact', 'someone', 'talk.*person', 'real.*person'], reply: '**Need a real person?** We\'ve got you.\n\nOur support team is active **24/7** on Discord. Just open a ticket and someone will respond quickly — no bots, no automated replies.\n\n[Open a support ticket](https://discord.com/channels/@me)' },
    { keys: ['hi', 'hello', 'hey', 'sup', "what's up", 'how are you', 'yo', 'wassup', 'good morning', 'good night'], reply: '**Hey there!** 👋 Welcome to Ghxstly Store.\n\nI\'m here to help you with anything about the store — prices, accounts, codes, delivery, warranties, you name it.\n\nWhat would you like to know?' },
    { keys: ['skin', 'outfit', 'cosmetic', 'galaxy', 'og', 'renegade', 'travis', 'black knight', 'aura', 'raider'], reply: '**Looking for specific skins?**\n\nCheck our listings — each account shows its skin count and tier. Want something specific like Travis Scott, Renegade Raider, or Black Knight?\n\nPress **Custom Account** to request exactly what you want. We\'ll build it for you!' },
    { keys: ['account.*type', 'bronze', 'silver', 'gold', 'platinum', 'diamond', 'tier', 'what.*tier'], reply: '**Account tiers by skin count:**\n\n🥉 **Bronze:** 1–50 skins — great starter accounts\n🥈 **Silver:** 50–100 skins — solid collection\n🥇 **Gold:** 100–200 skins — serious locker\n💎 **Platinum:** 200–400 skins — elite accounts\n👑 **Diamond:** 400+ skins — top-tier monsters\n\nAll accounts are capped at **$60** — the best value you\'ll find anywhere.' },
    { keys: ['how many', 'stock', 'available', 'out of stock', 'restock', 'when.*restock', 'sold out'], reply: 'We currently have **23 accounts** in stock right now.\n\nSold out on something you want? New accounts drop during **TikTok lives** and are announced in Discord. Follow https://www.tiktok.com/@ghxstlyfn for instant restock alerts.' },
    { keys: ['pay.*method', 'payment', 'card', 'paypal', 'crypto', 'apple pay', 'venmo', 'cashapp', 'zelle'], reply: '**Payment is simple:**\n\nWe use a **wallet system** — no credit card or PayPal needed.\n\n1. Get a recharge code from the store owner, TikTok lives, or giveaways\n2. Add it to your wallet\n3. Use your balance to buy accounts\n\nIt\'s fast, secure, and keeps everything in one place.' },
    { keys: ['thank', 'thanks', 'thx', 'ty', 'appreciate', 'helpful'], reply: '**You\'re welcome!** Happy to help. 😊\n\nIf you need anything else — buying help, account questions, or just want to chat — I\'m always here. Enjoy your new locker!' },
    { keys: ['night', 'late', 'hours', 'open', 'available', 'when.*open', '24', 'always', 'time'], reply: '**We\'re always open!** The store runs 24/7 online.\n\nHuman support on Discord is available day and night — just open a ticket and someone will respond.\n\n[Open a ticket](https://discord.com/channels/@me)' },
    { keys: ['fortnite', 'fn', 'epic', 'epic games', 'battle royale'], reply: '**Ghxstly Store** is an independent marketplace for Fortnite accounts. We\'re not affiliated with or endorsed by Epic Games.\n\nAccounts are traded peer-to-peer with full buyer protection, 48-hour warranty, and instant delivery.' },
    { keys: ['account', 'login', 'email', 'password', 'creds', 'credential', 'log in', 'sign in'], reply: '**After you buy an account:**\n\n1. You\'ll see the email and password in the **delivery modal**\n2. Copy them and log in to Fortnite\n3. If you lose them, open a Discord ticket with your order code\n\nCredentials are **never visible before purchase** — they\'re protected server-side.' },
    { keys: ['swap', 'trade', 'exchange', 'switch'], reply: 'We don\'t do swaps or trades — we **sell accounts directly** at the best prices you\'ll find. Browse our listings and find the perfect locker for you.' },
    { keys: ['fast', 'quick', 'instant', 'speed', 'how long', 'wait', 'long.*time'], reply: '**Lightning fast delivery:**\n\n• Order codes are issued **instantly** when you pay\n• Account handover happens in your Discord ticket\n• Most handovers are completed within **minutes**\n\nNo waiting. No delays. You\'re playing on your new account today.' },
    { keys: ['ban', 'risk', 'get banned', 'will i get banned', 'safe.*buy'], reply: '**Account trading carries some risk** — Epic Games\' Terms of Service may penalize it.\n\nHowever, we take every precaution:\n• 48-hour warranty on all accounts\n• Replacement or refund if anything goes wrong\n• Private, secure handovers\n\nBuy at your own discretion, but we\'ve got your back if anything happens.' },
    { keys: ['young', 'kid', 'child', 'age', 'old', 'minor'], reply: '**No age restrictions** on the store itself.\n\nYou just need a **Discord account** for delivery. If you need help setting up Discord or have questions, open a ticket and we\'ll walk you through it.' },
    { keys: ['multiple', 'bulk', 'buy more', 'two', 'three', '4', 'several'], reply: '**Buy as many accounts as you want!**\n\nAdd them to your cart and checkout all at once. Each account gets its own order code for separate delivery.\n\nPro tip: Use the **cart** feature to buy multiple accounts in one go!' },
    { keys: ['fast.*response', 'rude', 'bad.*service', 'wait.*long', 'slow', 'bad.*support'], reply: '**We pride ourselves on fast support.** If you\'re having a bad experience, open a ticket and tag the owner — we\'ll make it right.\n\n[Open a ticket](https://discord.com/channels/@me)' },
    { keys: ['update', 'change', 'edit', 'modify', 'update.*account'], reply: '**Once purchased, the account is yours.** If there\'s an issue within 48 hours, open a ticket for a replacement or full refund.' },
    { keys: ['what.*can.*you.*help', 'help.*me', 'options', 'menu', 'commands'], reply: '**Here\'s everything I can help with:**\n\n• 💰 **Prices** — cheapest, most expensive, price ranges\n• 🛒 **Buying** — step-by-step guide\n• 💳 **Wallet** — how to add balance\n• 📦 **Delivery** — order codes and handover\n• 🛡️ **Warranty** — 48-hour protection\n• 🏷️ **Promo codes** — discounts at checkout\n• 🎮 **Custom accounts** — build your dream locker\n• 🏆 **Tournaments** — cash prizes\n• 🎁 **TikTok lives** — giveaways and drops\n\nJust ask me anything!' },
    { keys: ['discord', 'server', 'join.*discord', 'link.*discord'], reply: '**Join our Discord server** for support, giveaways, and live updates!\n\n[Join Discord](https://discord.gg/ghxstly)\n\nOpen a ticket anytime for instant help from a real person.' },
    { keys: ['what.*happen', 'what.*go', 'explain', 'tell.*more', 'details'], reply: '**Here\'s the full picture:**\n\nGhxstly Store is a Fortnite account marketplace. We sell stacked accounts with tons of skins at affordable prices ($2-$60). Every purchase comes with a 48-hour warranty, instant delivery via Discord, and real human support 24/7.\\n\nBrowse the listings, pick an account, and we\'ll get you set up in minutes.' },
    { keys: ['compete', 'against', 'better.*than', 'other.*store', 'why.*you', 'why.*choose'], reply: '**Why Ghxstly Store?**\n\n• ✅ **Hard price cap** — never more than $60\n• ✅ **Instant delivery** — order codes in seconds\n• ✅ **48-hour warranty** — replacement or refund\n• ✅ **Real human support** — 24/7 on Discord\n• ✅ **Server-side security** — nothing can be faked\n• ✅ **TikTok lives** — giveaways and free codes\n\nWe\'re not the biggest store — we\'re the most trustworthy.' },
  ];
  ...
  window.openSupportChat = function () {
    ...
    if ($('#support-messages').children.length === 0) {
      appendSupportMessage('Hey there! 👋 I\'m the Ghxstly Store assistant.\n\nI can help you with:\n• 💰 Prices and accounts\n• 🛒 How to buy\n• 💳 Wallet codes\n• 📦 Delivery info\n• 🛡️ Warranty & refunds\n• 🏷️ Promo codes\n• 🎮 Custom accounts\n• 🏆 Tournaments\n\nWhat would you like to know?', false);
    }
  };
  ...
  window.sendSupportMessage = function () {
    ...
    setTimeout(() => {
      ...
      if (faqReply) {
        ...
        appendSupportMessage('I\'m not sure about that one. Here are some things I can help with:\n\n• **Prices** — cheapest account is just $2!\n• **How to buy** — 3 simple steps\n• **Wallet codes** — how to add balance\n• **Delivery** — instant order codes\n• **Warranty** — 48-hour protection\n• **Promo codes** — % discounts at checkout\n• **Custom accounts** — build your dream locker\n• **Tournaments** — cash prizes\n\nOr talk to a real person: [Open Ticket](https://discord.com/channels/@me)', false);
      }
      input.disabled = false;
      input.focus();
    }, 500);
